...
// 推送到 Telegram Bot
async function pushToTelegramBot(newItems, timeRange) {
...
// 共享的安全内容处理函数
async function handleSecurityFeed(req, res) {
    try {
        ...
        if (isCacheValid) {
            ...
            if (needsBackgroundUpdate && !isUpdating) {
                updateCacheInBackground().catch(err => console.error('Background update failed:', err));
            }
        ...
        if (newItems.length > 0) {
            const timeRange = lastTelegramPushTime > 0
                ...
                : '最近 30 分钟';
            // 异步推送，不阻塞响应
            pushToTelegramBot(newItems, timeRange).catch(err => console.error('Telegram push failed:', err));
        }
...
function renderSecurityPage(res, items, fromCache, categoryFilter) {
    // 确定要使用的数据源（优先使用缓存）
